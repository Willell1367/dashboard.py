...
def render_enhanced_performance_metrics(performance: Dict, bot_id: str):
    """CLEANED: Enhanced performance metrics - NO DUPLICATES"""
    ...
    # RISK-ADJUSTED METRICS ROW (4 columns)
...
class DashboardData:
    """CLEANED: Centralized data management - removed cache decorators"""
    ...
    def get_live_performance(self, bot_id: str) -> Dict:
        """CLEANED: Get live performance - removed problematic cache"""
...
def main():
    """CLEANED: Main dashboard application - streamlined"""
    ...
    if selected_view == "PORTFOLIO":
        ...
        # Live Trade Feed
        st.markdown("---")
        render_live_trade_feed(fills, selected_view)
...
if __name__ == "__main__":
    main() = datetime.strptime(ONDO_START_DATE, "%Y-%m-%d").timestamp() * 1000
    ...
    def _get_fallback_performance(self, bot_id: str) -> Dict:
        ...
        if bot_id == "ETH_VAULT":
        ...
        else:  # ONDO_PERSONAL - Current actual stats
            ...
            return {
                ...
                'max_drawdown': 0.0,
                'cagr': 0.0,
                'avg_daily_return': (1.94 / ONDO_PERSONAL_START_BALANCE) * 100,
                'total_return': (1.94 / ONDO_PERSONAL_START_BALANCE) * 100,
                'trading_days': trading_days
            }
    
    def get_live_position_data(self, bot_id: str) -> Dict:
        """CLEANED: Get live position data - removed cache"""
        try:
            bot_config = self.bot_configs[bot_id]
    ...
    def get_bot_fills(self, bot_id: str) -> List[Dict]:
        """CLEANED: Get trade fills - removed cache"""
        try:
            bot_config = self.bot_configs[bot_id]
            ...
            if bot_id == "ETH_VAULT" and bot_config.vault_address:
                ...
                fills = self.api.get_fills(address)
                # Filter to ONDO only and after start date
                ondo_start_timestamp# Hyperliquid Trading Dashboard - Enhanced with Interactive Charts
# File: dashboard.py - CLEANED VERSION - Duplicates Removed
# Updated: Aug 15, 2025 - Removed all duplicates, optimized structure
